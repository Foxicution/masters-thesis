...
class GraphDataset(Dataset):
    ...
    def _init_mean_std_y(self):
        ...
        for commit_sha in self.targets:
            all_targets.append(
                torch.tensor(self.targets[commit_sha], dtype=torch.float32)
            )
...
def train_model(
    ...
    model.to(device)
    node_emb_model.to(device)
    edge_emb_model.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for i, graph in enumerate(
            tqdm(train_dataset, desc=f"Epoch {epoch+1}/{num_epochs}")
        ):
